...
// =========================
// SEO SECTION – PAGE METADATA (Archegos Case)
// =========================
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "cases" });
  const baseKey = "archegos";

  const title = t(`${baseKey}.title`) || "Archegos Case";
  const description = t(`${baseKey}.lead`) || defaultDescription;
  const alternates = buildAlternates({ locale, path: "/archegos" });
...
export default async function ArchegosCasePage({
  params,
}: {
